...
describe('result and route state', () => {
  ...
  it('places the configured currency symbol before or after monetary values', () => {
    ...
    expect(configuredCurrencyDecimals('EUR', currencies)).toBe(2)
    expect(formatConfiguredCurrency(-15.7786, 'EUR', {}, currencies)).toBe('-€15.78')
    expect(formatConfiguredCurrency(
      -15.7786,
      'EUR',
      { currency_prefix: false },
      currencies
    )).toBe('-15.78 €')
    expect(formatConfiguredCurrency(15.7786, 'KWD', {}, currencies)).toContain('15.779')
  })
