...
export function configuredCurrencyDecimals(currency = 'USD', currencies = []) {
  const code = String(currency || 'USD').toUpperCase()
  const configured = currencies.find(item => String(item?.code || '').toUpperCase() === code)
  const decimals = Number(configured?.decimals)
  if (Number.isInteger(decimals) && decimals >= 0 && decimals <= 20) return decimals
  try {
...
export function formatConfiguredCurrency(value, currency = 'USD', display = {}, currencies = []) {
  const parsed = Number(value)
  const amount = Number.isFinite(parsed) ? parsed : 0
  const code = String(currency || 'USD').toUpperCase()
  const decimals = configuredCurrencyDecimals(code, currencies)
