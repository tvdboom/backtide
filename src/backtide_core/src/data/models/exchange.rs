...
impl Exchange {
    fn data(&self) -> (&'static str, Country, &'static str, &'static str, Currency) {
        ...
        match self {
            ...
            XATH => ("Athens Exchange", GRC, "Athens", "ATH", Currency::EUR),
            XASX => ("Australian Securities Exchange", AUS, "Sydney", "ASX", AUD),
            XBKK => ("Stock Exchange of Thailand", THA, "Bangkok", "SET", THB),
            XBOG => ("Colombia Stock Exchange", COL, "Bogotá", "BVC", COP),
            XBOM => ("Bombay Stock Exchange", IND, "Mumbai", "BSE", INR),
            XBRU => ("Euronext Brussels", BEL, "Brussels", "BRU", Currency::EUR),
            XBUD => ("Budapest Stock Exchange", HUN, "Budapest", "BUD", HUF),
            XBUE => ("Buenos Aires Stock Exchange", ARG, "Buenos Aires", "BUE", ARS),
            XCAI => ("Egyptian Exchange", EGY, "Cairo", "CAI", EGP),
            ...
            XDUB => ("Euronext Dublin", IRL, "Dublin", "ISE", Currency::EUR),
            ...
            XICE => ("Nasdaq Iceland", ISL, "Reykjavik", "ICE", ISK),
            XIDX => ("Indonesia Stock Exchange", IDN, "Jakarta", "JKT", IDR),
            XIST => ("Borsa Istanbul", TUR, "Istanbul", "IST", TRY),
            XJPX => ("Japan Exchange Group", JPN, "Tokyo", "JPX", JPY),
            XKAR => ("Pakistan Stock Exchange", PAK, "Karachi", "KAR", PKR),
            XKLS => ("Bursa Malaysia", MYS, "Kuala Lumpur", "KLS", MYR),
            XKRX => ("Korea Exchange", KOR, "Seoul", "KSC", KRW),
            XKUW => ("Kuwait Stock Exchange", KWT, "Kuwait City", "KUW", KWD),
            XLIM => ("Lima Stock Exchange", PER, "Lima", "LM", PEN),
            XLIS => ("Euronext Lisbon", PRT, "Lisbon", "LIS", Currency::EUR),
            XLIT => ("Nasdaq Vilnius", LTU, "Vilnius", "LIT", Currency::EUR),
            ...
            XMEX => ("Mexican Stock Exchange", MEX, "Mexico City", "MEX", MXN),
            XMIL => ("Borsa Italiana", ITA, "Milan", "MIL", Currency::EUR),
            XMOS => ("Moscow Exchange", RUS, "Moscow", "MCX", RUB),
            ...
            XPRA => ("Prague Stock Exchange", CZE, "Prague", "PRA", CZK),
            XRIS => ("Nasdaq Riga", LVA, "Riga", "RIS", Currency::EUR),
            XSAU => ("Saudi Exchange", SAU, "Riyadh", "SAU", SAR),
            XSES => ("Singapore Exchange", SGP, "Singapore", "SES", SGD),
            XSGO => ("Santiago Stock Exchange", CHL, "Santiago", "SGO", CLP),
            XSHE => ("Shenzhen Stock Exchange", CHN, "Shenzhen", "SHZ", CNY),
            XSHG => ("Shanghai Stock Exchange", CHN, "Shanghai", "SHH", CNY),
            XSTC => ("Ho Chi Minh Stock Exchange", VNM, "Ho Chi Minh City", "VSE", VND),
            XSTO => ("Nasdaq Stockholm", SWE, "Stockholm", "STO", SEK),
            XSWX => ("SIX Swiss Exchange", CHE, "Zurich", "EBS", CHF),
            XTAI => ("Taiwan Stock Exchange", TWN, "Taipei", "TAI", TWD),
            XTAL => ("Nasdaq Tallinn", EST, "Tallinn", "TAL", Currency::EUR),
            XTSX => ("Toronto Stock Exchange", CAN, "Toronto", "TOR", CAD),
            XWAR => ("Warsaw Stock Exchange", POL, "Warsaw", "WSE", PLN),
...
impl_python_enum_variants!(Exchange);
