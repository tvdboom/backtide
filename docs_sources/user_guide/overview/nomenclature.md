...
[](){#nom-backtest}
<strong id="backtest">backtest</strong>
<div markdown style="margin: -1em 0 0 1.2em">
A simulation that applies a strategy to historical market data under explicit execution and risk
assumptions. A backtest tests how rules would have behaved; it does not predict future returns.
</div>
...
[](){#nom-experiment}
<strong id="experiment">experiment</strong>
<div markdown style="margin: -1em 0 0 1.2em">
One reproducible backtest containing its data range, strategies, indicators, metrics, portfolio,
and execution assumptions. An experiment produces one [run][nom-run] per strategy.
</div>
...
[](){#nom-metric}
<strong id="metric">metric</strong>
<div markdown style="margin: -1em 0 0 1.2em">
A finite scalar that summarizes one completed strategy run, such as total return, Sharpe ratio, or
maximum drawdown. Read more in the [user guide][metrics].
</div>

<br>

[](){#nom-order}
<strong id="order">order</strong>
<div markdown style="margin: -1em 0 0 1.2em">
A strategy instruction to buy, sell, cancel, or settle an instrument position. Its signed quantity
and order type determine the requested direction and execution behavior. See [`Order`].
</div>
