...
5. **Run strategies.** Each strategy gets its own portfolio, order book, equity log,
   and trade log.
...
- Auto-injects an extra strategy run named `Benchmark` that holds a pure passive
  ...
  [alpha](../library/metrics.md#alpha-and-excess-return). The benchmark run ignores starting positions.
...
The [Results guide](results.md) shows how to extract these records from the returned objects. See
[Metrics](../library/metrics.md#formulas) for the definitions and formulas behind the summary values.
