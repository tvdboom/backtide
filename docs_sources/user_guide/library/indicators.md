...
potential reversal points. Backtide provides a set of built-in indicators as well as a framework
for creating custom indicators in Python.

<br>

## How they work

Every indicator inherits from [`BaseIndicator`] and implements `compute(self, data)`:

- `data` — `pandas.DataFrame | polars.DataFrame`. It contains chronological OHLCV bars and uses
  the dataframe library selected in the [configuration][configuration]. For example,
  `data["close"]` selects the complete close-price column and `data[["high", "low", "close"]]`
  selects three inputs in pandas.
- Return value — `pandas.Series | pandas.DataFrame | polars.Series | polars.DataFrame`. Return a
  series for one output, such as `data["close"].rolling(20).mean()`, or a dataframe for multiple
  outputs, such as upper, middle, and lower bands.
...
) -> pandas.Series | pandas.DataFrame | polars.Series | polars.DataFrame:
    ...
```

Output shapes determine how the result is used:
...
class MyIndicator(BaseIndicator):
    def compute(self, data):
        # `data` is the configured pandas or Polars OHLCV dataframe.
...
See [Typical price](../../examples/indicators/typical_price.md) and
[Rolling price range](../../examples/indicators/rolling_price_range.md) for complete custom
indicators that work with both supported dataframe backends. For costly numeric calculations,
read [Performance](performance.md#indicator-performance).

<br>

## Built-in indicators

Built-in indicators accept OHLCV data in any configured [`DataFrameLibrary`] format and return
