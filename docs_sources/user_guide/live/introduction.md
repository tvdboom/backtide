...
Backtide's live workflow is **paper trading**: it consumes current WebSocket market updates, runs a
strategy, and simulates orders, fills, equity, and metrics locally. It does not connect to a broker
or submit real orders.

Paper trading is the bridge between a historical backtest and real-world observation. It lets you
...
3. Reuse the strategy, indicators, sizer, fees, and risk settings from the experiment where
   possible.
4. Start a paper session and monitor connection state, orders, fills, positions, and metrics.
...
Continue with the [Paper trading guide](paper_trading.md) for the application, CLI, and Python
interfaces. The [complete session example](../../examples/live/paper_trading_session.md) shows the
full lifecycle in one script.

!!! note
    Paper results can differ from a backtest because the observed period, update timing, and
